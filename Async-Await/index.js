
function bestRockBand(band) {
    return new Promise((resolve, reject) => {
        if (band == 'Queen'){
            resolve({
                success: true,
                bandName: band,
                msg: band + ' is the best rock band ever!'
            })
        } else {
            reject({
                success: false,
                msg: 'I\'m not sure!'
            })
        }
    })
}

function bestRockSong(response) {
    return new Promise((resolve, reject) => {
        if(response.success){
            resolve('Bohemian Rhapsody by ' + response.bandName + '. ' + response.msg);
        } else {
            reject('Do you know Queen?');
        }
    })
}

/*bestRockBand('Queen')
    .then(response => {
        console.log('Cheking the answer...');
        return bestRockSong(response)
    })
    .then(response => {
        console.log('Finding the best song...');
        console.log(response);
    })
    .catch(err => {
        console.log(err.msg)
    })
*/

// Um await é usado antes da chamada de uma promise, e so pode ser usado em funções assincronas

// Para usarmos o recurso do cacth dentro da função assincrona é usando o Try Catch, assim teremos a reposta do reject da promise, não so do resolve.

async function doTheJob(){
    try {
        const bestRockBandResponse = await bestRockBand('Kiss');
        console.log(bestRockBandResponse);
        const bestRockSongResponse = await bestRockSong(bestRockBandResponse);
        console.log(bestRockSongResponse);
    } catch (err) {
        console.log(err.msg)
    }
    
}

doTheJob();